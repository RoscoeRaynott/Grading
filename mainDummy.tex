...
In the second experiment, we fix the noise level (standard deviation of Gaussian errors) and vary the number of observations $n \in \{30, 60, 100, 200, 300, 500, 800, 1000\}$. This allows us to study how each method's prediction performance scales with sample size at a given noise regime. We consider three fixed noise levels --- low ($\sigma = 0.5$), medium ($\sigma = 1.5$), and high ($\sigma = 2.5 $) --- for both the Length and Amplitude datasets.
...
For both the datasets, the general trend is that as $n$ increases at each fixed noise level, SI-ScoSH and Kernel Regression with Shape distance converge in performance. Kernel Regression with $L^2$ distance remains consistently below both shape-aware methods across all sample sizes and noise levels.

(make curves smoother/ draw inference.. parametric bettter than non- in small n scene)
