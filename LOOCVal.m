...
% % end % % % % % Ensure y is a column vector % % y = y( :);
% % % % % % 2. Split Data(80 % Train, 20 % Test) % % %
    -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -%
    % fraction = 0.8;
% % n_train = round(fraction * n);
% % n_test = n - n_train;
% % % % % Random shuffle indices to ensure random split % % rng(1); % Fixed seed for reproducibility
% % rand_idx = randperm(n);
% % % % idx_train = rand_idx(1 : n_train);
% % idx_test = rand_idx(n_train + 1 : end);
% % % % % Create Sets % % Q_train = qi(idx_train, :);
% % y_train = y(idx_train);
% % % % Q_test = qi(idx_test, :);
% % y_test = y(idx_test);
% % % % % % 3. Define Distance Function(Placeholder) % % %
    -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -%
    % % You said "Leave distance calc to me",
...
% % end % % end % % end % % % % %
    ...
    % J = Sum(y_i - y_hat_i) ^
          2 % % % y_hat_i is calculated using all points except i %
              % loocv_cost_fun = @(tau)
              calculate_loocv_error(tau, DistMat_Train, y_train);
% % 
...
% % end % % % % % 2. Calculate Kernels(Un - normalized weights) %
    % u_vec = exp(-dists_sq / tau_optimal);
% % % % % 3. Normalize Weights % % w_vec = u_vec / sum(u_vec);
% % % % % 4. Weighted Sum Prediction % % y_test_pred(k) = sum(w_vec.*y_train);
% % end % % % % % % 6. Results Visualization % % %
    -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -%
    % figure;
...
% % % % % % % % -- -HELPER FUNCTION : LOOCV COST-- - % % function J =
    calculate_loocv_error(tau, D_sq, y) % % %
    D_sq : Squared Distance Matrix(Pre - calculated) % % %
           y : Training responses % % % % n = length(y);
% % J = 0;
% % % % % Calculate Kernel Matrix % % K = exp(-D_sq / tau);
% % % % %
    IMPORTANT
    : Set diagonal to 0. %
      % %
...
% %     sum_weights = sum(K, 2);
% % % % % Avoid division by zero % % sum_weights(sum_weights == 0) = eps;
% % % % % Normalized Weights Matrix % % W = K./ sum_weights;
% % % % % Predictions : y_hat = W *y % % y_hat = W * y;
% % % % % Sum of Squared Errors % % J = sum((y - y_hat).^ 2);
% % end % % % % clear all;
% % % % close all;
% % % % % % % % % % 1. Data Generation % % % % gap = 0.02;
% % % % t = 0 : gap : 1;
% % % % n = 100;
% % % % stdER = 0.01;
% % % % ScoSh = 0;
% % % % % % % % % Pre - allocate F to store original curves % % % % F =
    zeros(n, length(t));
% % % % qi = zeros(n, length(t));
% % % % y = zeros(n, 1);
% % % % costtrue = 0;
...
% % % %         qi(i,:)=curve_to_q(fi{i}(t));
% % % % else % % % % % Fallback if function missing % % % % qi(i, :) = f(i, :);
% % % % end % % % % % % % % epsi(i) = normrnd(0, stdER);
% % % % y(i) = epsi(i) + max(f(i, :)) - min(f(i, :));
% sum(abs(diff([0 fi{i}(t)]) / gap) * gap);
% % % % % costtrue = costtrue + epsi(i) ^ 2;
% % % % end % % % % y = y( :);
% % % % % % % % % % 2. Split Data(80 % Train, 20 % Test) % % % % fraction = 0.8;
% % % % n_train = round(fraction * n);
% % % % n_test = n - n_train;
% % % % rng(1);
% % % % rand_idx = randperm(n);
...
end % % % % % % % % disp('--- Running Method 1: Standard L2 (ScoSh = 0) ---');
% % % % [ tau_0, y_pred_0, R2_0 ] = run_regression_pipeline(
    Q_train, F_train, y_train, Q_test, F_test, y_test, t, 0);
% % % % % % % % disp('--- Running Method 2: Shape Alignment (ScoSh = 1) ---');
% % % % [ tau_1, y_pred_1, R2_1 ] = run_regression_pipeline(
    Q_train, F_train, y_train, Q_test, F_test, y_test, t, 1);
...
% % % %
    plot(limits, limits, 'k--', 'LineWidth', 2, 'DisplayName',
         'Perfect Prediction');
% % % % % % % % % 2. Plot Standard L2 Results(Blue Circles) % % % %
    plot(y_test, y_pred_0, 'bo', 'LineWidth', 1.5, 'DisplayName',
         ['Standard L2 (R^2=' num2str(R2_0, '%.2f') ')']);
% % % % % % % % % 3. Plot Shape Aligned Results(Red Crosses) % % % %
    plot(y_test, y_pred_1, 'rx', 'LineWidth', 1.5, 'DisplayName',
         ['Shape Aligned (R^2=' num2str(R2_1, '%.2f') ')']);
% % % % % % % % % 4. Formatting % % % % legend('Location', 'best');
...
% -- -Setup Sample Sizes to Test-- - n_vals = [
...
fprintf('\nProcessing n = %d ...\n', n);
    ...
    % Apply current stdER here epsi(i) = normrnd(0, stdER);
    y(i) = epsi(i) + sum(abs(diff([0 fi{i}(t)]) / gap) * gap);
    end y = y( :);

    % -- -SPLIT DATA-- - fraction = 0.75;
    n_train = round(fraction * n);
    rng(1);
    % Keep split consistent rand_idx = randperm(n);
    idx_train = rand_idx(1 : n_train);
    idx_test = rand_idx(n_train + 1 : end);
    ...
    % -- -RUN ANALYSIS-- - % Method 1
        : Standard L2(ScoSh = 0)[~, ~, R2_0] = run_regression_pipeline(
        Q_train, F_train, y_train, Q_test, F_test, y_test, t, 0);

    % Method 2 : Shape Aligned(ScoSh = 1)[~, ~, R2_1] = run_regression_pipeline(
        Q_train, F_train, y_train, Q_test, F_test, y_test, t, 1);

    % Store Results results_table(s_idx, :) = [ n, R2_0, R2_1 ];
    end
    ...
    fprintf('-----------------------------------------\n');
for
  i = 1 : n_trials fprintf(' %-10d | %-12.4f | %-12.4f \n',
                           ... results_table(i, 1), results_table(i, 2),
                           results_table(i, 3));
end fprintf('-----------------------------------------\n');

% % Plot L2 &Shape lines on shared figure 6769 figure(6769);
hold on;
plot(n_vals, results_table( :, 2), 's-', 'Color', [0.4660 0.6740 0.1880],
     ... 'LineWidth', 2, 'MarkerSize', 10, 'DisplayName', 'K.reg (L_2 dist)');
plot(n_vals, results_table( :, 3), 'd-', 'Color', [0.8500 0.3250 0.0980],
     ... 'LineWidth', 2, 'MarkerSize', 10, 'DisplayName', 'K.reg (Shape dist)');

% % Apply final formatting to figure 6769 ax = gca;
legendNames = {'SI-ScoSH', 'K.reg (Shape dist)', 'K.reg (L_2 dist)'};
lineColors = {
    [0 0.4470 0.7410], [0.8500 0.3250 0.0980], [0.4660 0.6740 0.1880]};
lines = findobj(ax, 'Type', 'Line');
for
  j = 1 : min(length(lines), 3) lines(j).LineWidth = 2.0;
...
% (Keep your helper functions below this line) % % -- -HELPER FUNCTIONS-- -

    function[tau_opt, y_pred, R2] =
    run_regression_pipeline(Q_tr, F_tr, y_tr, Q_te, F_te, y_te, t, ScoSh)
        n_train = size(Q_tr, 1);
n_test = size(Q_te, 1);

% A.Calculate Training Distance Matrix DistMat = zeros(n_train, n_train);

parfor i = 1 : n_train row_dists = zeros(1, n_train);
        for
          j = (i + 1)
              : n_train % Pass ScoSh to the distance function row_dists(j) =
                  get_warped_dist_sq(Q_tr(i, :), Q_tr(j, :), F_tr(i, :), t,
                                     ScoSh);
        end DistMat(i, :) = row_dists;
        end DistMat = DistMat + DistMat'; % Mirror symmetry

                                    % B.Optimize Tau loocv_fun =
                          @(tau) calculate_loocv_error(tau, DistMat, y_tr);
        tau_opt = fminbnd(loocv_fun, 0.001, 100);

        % C.Predict on Test Set y_pred = zeros(n_test, 1);

        parfor k = 1 : n_test q_new = Q_te(k, :);
        f_new = F_te(k, :);
        dists_sq = zeros(n_train, 1);

        for
          i = 1 : n_train dists_sq(i) =
                      get_warped_dist_sq(q_new, Q_tr(i, :), f_new, t, ScoSh);
        end
        ...
        y_pred(k) = sum(w_vec.*y_tr);
                ...
                % D.Calculate R ^
            2 SS_res = sum((y_te - y_pred).^ 2);
        SS_tot = sum((y_te - mean(y_te)).^ 2);
        R2 = 1 - (SS_res / SS_tot);
        end

            % PERFORMANCE OPTIMIZED DISTANCE FUNCTION function dist_sq =
            get_warped_dist_sq(q_a, q_b, f_a, t, ScoSh) if ScoSh ==
            1 % Expensive Alignment Path gam =
                DynamicProgrammingQ_Adam(q_a, q_b, 0, 0);
        f_a_warped = interp1(t, f_a, gam, 'linear', 'extrap');
        q_a_prime = curve_to_q(f_a_warped);
        dist_sq = sum((q_a_prime - q_b).^ 2);
        else % Fast L2 Path(Skips DP and Interp) dist_sq = sum((q_a - q_b).^ 2);
        end end

            function J = calculate_loocv_error(tau, D_sq, y) n = length(y);
        K = exp(-D_sq / tau);
        K(logical(eye(n))) = 0;
        sum_weights = sum(K, 2);
