...
In the second experiment, we fix the noise level (standard deviation of Gaussian errors) and vary the number of observations $n \in \{30, 60, 100, 200, 300, 500\}$. This allows us to study how each method's prediction performance scales with sample size at a given noise regime. We consider three fixed noise levels --- low ($\sigma = \sigma_1$), medium ($\sigma = \sigma_2$), and high ($\sigma = \sigma_3$) --- for both the Length and Amplitude datasets.

\begin{figure}[h!]
    \centering
    \setlength{\tabcolsep}{2pt}
    \begin{tabular}{cc}
        \includegraphics[width=0.45\textwidth]{NvsPerfLen_stder1.eps} &
        \includegraphics[width=0.45\textwidth]{NvsPerfAmp_stder1.eps} \\
        (a) Dataset Length, $\sigma = \sigma_1$ & (b) Dataset Amplitude, $\sigma = \sigma_1$ \\[6pt]
        \includegraphics[width=0.45\textwidth]{NvsPerfLen_stder2.eps} &
        \includegraphics[width=0.45\textwidth]{NvsPerfAmp_stder2.eps} \\
        (c) Dataset Length, $\sigma = \sigma_2$ & (d) Dataset Amplitude, $\sigma = \sigma_2$ \\[6pt]
        \includegraphics[width=0.45\textwidth]{NvsPerfLen_stder3.eps} &
        \includegraphics[width=0.45\textwidth]{NvsPerfAmp_stder3.eps} \\
        (e) Dataset Length, $\sigma = \sigma_3$ & (f) Dataset Amplitude, $\sigma = \sigma_3$
    \end{tabular}
...
% TODO: Replace with final analysis once inferences are ready.
For the Length dataset, the general trend is that as $n$ increases at each fixed noise level, SI-ScoSH and Kernel Regression with Shape distance converge in performance. Kernel Regression with $L^2$ distance remains consistently below both shape-aware methods across all sample sizes and noise levels.
